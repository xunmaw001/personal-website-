    ...
    >
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="视频详情" prop="shipinxiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.shipinxiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shipinxiangqing" label="视频详情" prop="shipinxiangqing">
                    <span v-html="ruleForm.shipinxiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shipin = true;
          ...
          if(o=='shipinxiangqing'){
            this.ruleForm.shipinxiangqing = obj[o];
	    this.ro.shipinxiangqing = true;
            continue;
          }
          if(o=='faburen'){
	...
	this.ruleForm.shipinxiangqing = this.ruleForm.shipinxiangqing.replace(reg,'../../../ssm1l451/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shipinku/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shipinkuCrossAddOrUpdateFlag = false;
                  this.parent.search();
